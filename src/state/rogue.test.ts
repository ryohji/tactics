...
describe('両手持ち・盾(rogue-22)', () => {
  ...
  it('セーブ v11: player.shield が保存・復元される(rogue-30)', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
...
describe('スキル: マスタリー×スロット(rogue-23。rogue-27でランク・結び・排他・天秤ドラフトへ改訂)', () => {
  ...
  it('セーブ v11: skillSlots/skillEquipped(ランク付き)/skillDraft/skillFreePick/cooldowns が保存・復元される(rogue-35)', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
      useRogue.setState({
...
describe('スキルのクールダウン一般化(rogue-30)', () => {
  ...
  it('セーブ v11: cooldowns.rengeki も保存・復元される', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
...
describe('saveCodec v11(rogue-35)', () => {
  it('v11 でラウンドトリップ: relics が保存・復元される', () => {
