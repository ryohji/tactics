...
import { cellKey, keyToCell, layer, neighbors, OFFSETS, type Cell } from '../model/fcc';
...
/** 全カウンタを大きな値にした MasteryCounters(rogue-35: 全ノード全ランク解禁の土台)。 */
...
/**
 * ずらし動詞(rogue-35: 盾打ち・突進)のテスト用: player.pos([0,0,0]、restart 直後の既定位置)
 * から、ある方向(OFFSETS の1つ)へ steps+1 マス連続で開いている経路を探す(複数シードを
 * 試す)。見つかった方向 dir・1歩目の位置(隣接。ここに敵を置く)・2歩目以降の位置列
 * (steps 個。押し先/移動先の候補)を渡してテスト本体を実行する。
 */
function withStraightOpenPath(
  steps: number,
  body: (dir: Cell, adjacent: Cell, farCells: Cell[]) => void,
): void {
  for (let seed = 1; seed <= 60; seed++) {
    useRogue.getState().restart(seed);
    const s = useRogue.getState();
    for (const o of OFFSETS) {
      const adjacent: Cell = [o[0], o[1], o[2]];
      if (!s.dungeon.open.has(cellKey(adjacent))) continue;
      const farCells: Cell[] = [];
      let ok = true;
      for (let i = 2; i <= steps + 1; i++) {
        const c: Cell = [o[0] * i, o[1] * i, o[2] * i];
        if (!s.dungeon.open.has(cellKey(c))) {
          ok = false;
          break;
        }
        farCells.push(c);
      ...
      if (ok) {
        seedRogueRng(seed);
        body(o, adjacent, farCells);
        return;
      }
    }
  }
  throw new Error('直線に開けた経路が見つからなかった(seed探索を広げる必要がある)');
}
...
describe('敵の障壁(rogue-36)', () => {
  /** state 内の敵を id で取り直す(store は毎 set で配列を作り直すため)。 */
  const beast = (id: number) => useRogue.getState().beasts.find((x) => x.id === id)!;

  it('与ダメージはまず障壁から削れ、HP は障壁が残る間は減らない', async () => {
    const b = placeBeastAdjacent('drake', 20); // hp/maxHp 20・def3
    useRogue.setState({
      beasts: useRogue.getState().beasts.map((x) => (x.id === b.id ? { ...x, barrier: 20 } : x)),
    });
    useRogue.getState().clickBeast(b.id);
    await run();
    const after = beast(b.id);
    // 1発ぶんが障壁から抜け、HP は満タンのまま(障壁がまだ残る)。
    expect(after.barrier).toBeGreaterThan(0);
    expect(after.barrier).toBeLessThan(20);
    expect(after.hp).toBe(20);
    expect(after.alive).toBe(true);
  });

  it('障壁を削り切るとログが出て、以降のダメージは HP へ抜ける', async () => {
    const b = placeBeastAdjacent('bat', 30); // def0・hp/maxHp を高めに固定
    // 障壁1だけ張る → 1発目で必ず砕ける。
    useRogue.setState({
      beasts: useRogue.getState().beasts.map((x) => (x.id === b.id ? { ...x, barrier: 1 } : x)),
    });
    useRogue.getState().clickBeast(b.id);
    await run();
    expect(beast(b.id).barrier).toBe(0);
    expect(useRogue.getState().log.some((l) => l.includes('障壁が砕けた'))).toBe(true);
    // 障壁が無くなった状態でもう一撃 → 必ず HP へ抜ける。
    useRogue.getState().clickBeast(b.id);
    await run();
    expect(beast(b.id).hp).toBeLessThan(30);
  });
});
...
describe('両手持ち・盾(rogue-22)', () => {
  ...
  it('セーブ v12: player.shield が保存・復元される(rogue-30)', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
...
describe('スキル: マスタリー×スロット(rogue-23。rogue-27でランク・結び・排他・天秤ドラフトへ改訂)', () => {
  ...
  describe('支度(ラン開始直後の自由装着)', () => {
    ...
    it('解禁済みノードが1つ以上あれば restart 直後に開き、ゲーム操作をブロックする', () => {
      ...
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 5 }); // fistKills>0 → hasAnyMastery
      useRogue.getState().restart(7);
      const s = useRogue.getState();
    ...
    it('装着でき、コスト超過は拒否される。閉じると通常操作に戻る', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      // fistKills15 → kenPunch 全ランク解禁。oneHandFreeKills15 → ryote 解禁。
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 15, oneHandFreeKills: 15 });
      useRogue.getState().restart(7);
      useRogue.getState().equipSkill('kenPunch'); // ランク1(コスト1)
      useRogue.getState().equipSkill('kenPunch'); // ランク2に深める(差分コスト1、合計2・スロット3)
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 2 }]);
      useRogue.getState().equipSkill('ryote'); // コスト2、残り1では足りない → 拒否
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 2 }]);
      expect(useRogue.getState().log.at(-1)).toContain('足りない');
      useRogue.getState().unequipSkill('kenPunch');
    ...
    it('ランクアップ: 装着済みノードを再度 equipSkill するとコスト差分だけ消費し、ログは「深めた」', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 15 }); // kenPunch 全ランク解禁
      useRogue.getState().restart(7);
      useRogue.setState({ skillSlots: 8 });
      useRogue.getState().equipSkill('kenPunch'); // ランク1(コスト1)
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 1 }]);
      useRogue.getState().equipSkill('kenPunch'); // ランク2(差分コスト = costs[1]-costs[0] = 1)
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 2 }]);
      expect(useRogue.getState().log.at(-1)).toContain('深めた');
      expect(useRogue.getState().log.at(-1)).toContain('Ⅱ');
      useRogue.getState().equipSkill('kenPunch'); // ランク3(差分コスト = costs[2]-costs[1] = 1)
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 3 }]);
      expect(useRogue.getState().log.at(-1)).toContain('Ⅲ');
    });

    it('EXCLUDES: kenMuku ランクI と kenHaisui ランクIは同時装着できない', () => {
      ...
      useRogue.setState({
        skillSlots: 8,
        skillEquipped: [{ id: 'kenMuku', rank: 1 }],
        skillDraft: [{ id: 'kenHaisui', rank: 1, lane: 'nagare' }],
      });
      useRogue.getState().equipSkill('kenHaisui'); // EXCLUDES で拒否
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenMuku', rank: 1 }]);
  ...
  describe('関門ドラフト(天秤ドラフト+見送り権)', () => {
    ...
    it('マスタリーがあれば3択が出て操作をブロックし、同じマスタリー・同じ乱数列なら同じ候補になる(決定性)', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      masteryStore.writeMastery(fullMastery()); // 全カウンタ解禁
    ...
    it('ドラフトを見送ると何も装着されずに閉じ、見送り権が立つ(次の関門で free になる)', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 5 });
      useRogue.getState().restart(7);
      useRogue.getState().finishOutfitting();
      seedRogueRng(42);
      useRogue.setState({ player: { ...player(), pos: deep } });
    ...
    it("見送り権('free')中は takeable 全体から選べ、選ぶとドラフトが閉じる", () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 5 }); // kenPunch 解禁(根ノードなので0でも解禁済みだが明示)
      useRogue.setState({ skillDraft: 'free', skillSlots: 3, busy: true });
      useRogue.getState().equipSkill('kenPunch');
      expect(useRogue.getState().skillEquipped).toEqual([{ id: 'kenPunch', rank: 1 }]);
  ...
  describe('ノード効果の配線', () => {
    ...
    it('返し(kaeshi)ランクI: 回避成功時、隣接の攻撃者へ floor(攻撃力/3) の固定反撃(盾でも発動)', () => {
      ...
      useRogue.setState({
        ...
        skillEquipped: [{ id: 'kaeshi', rank: 1 }],
      });
      const atk = playerAtk(useRogue.getState().player, [{ id: 'kaeshi', rank: 1 }]);
      useRogue.getState().wait();
      expect(useRogue.getState().log.some((m) => m.includes('反撃'))).toBe(true);
      const beast = useRogue.getState().beasts.find((x) => x.id === b.id)!;
      expect(beast.hp).toBe(hp0 - Math.floor(atk / 3));
    });

    it('返し(kaeshi)ランクII/III: 反撃値は攻撃力の1/2・3/4', () => {
      for (const [rank, frac] of [[2, 1 / 2], [3, 3 / 4]] as const) {
        useRogue.setState({ beasts: [] }); // 前回ループの敵を消してから新規配置(id=900固定のため)
        const b = placeBeastAdjacent('bat');
        const hp0 = b.hp;
        useRogue.setState({
          player: {
          ...
          skillEquipped: [{ id: 'kaeshi', rank }],
        });
        const atk = playerAtk(useRogue.getState().player, [{ id: 'kaeshi', rank }]);
        useRogue.getState().wait();
        const beast = useRogue.getState().beasts.find((x) => x.id === b.id)!;
        expect(beast.hp).toBe(hp0 - Math.floor(atk * frac));
      }
    });

    it('返し(kaeshi)は素手・盾なしでも発動する(旧・見切りの統合)', () => {
      const b = placeBeastAdjacent('bat');
      const hp0 = b.hp;
      useRogue.setState({
        player: { ...player(), hp: 1, barrier: 0, weapon: null, shield: null }, // kenHaisui で盾なし回避+25%
        skillEquipped: [
          { id: 'kaeshi', rank: 1 },
          { id: 'kenHaisui', rank: 1 },
        ],
      ...
      let evaded = false;
      for (let i = 0; i < 60 && !evaded; i++) {
        seedRogueRng(i);
        useRogue.getState().wait();
        evaded = useRogue.getState().log.some((m) => m.includes('かわした'));
        if (useRogue.getState().phase !== 'play') break;
      }
      expect(evaded).toBe(true);
      const beast = useRogue.getState().beasts.find((x) => x.id === b.id);
      if (beast) expect(beast.hp).toBeLessThanOrEqual(hp0); // 反撃が乗れば減っている
    });
  ...
  describe('マスタリー(永続カウンタ v2・rogue-35)の加算', () => {
    it('素手討伐で拳闘の域に達したログが出る(fistKills: 0/5/15)', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 4 });
      const b = placeBeastAdjacent('bat', 1); // hp=1で確実に一撃で倒す
      useRogue.setState({ player: { ...player(), weapon: null } });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().fistKills).toBe(5);
      expect(useRogue.getState().log.some((m) => m.includes('の域に達した'))).toBe(true);
    });

    it('片手武器・盾なしでの討伐で oneHandFreeKills が加算される', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ player: { ...player(), weapon: { item: 'dagger', q: 0 }, shield: null } });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().oneHandFreeKills).toBe(1);
      expect(masteryStore.readMastery().twoHandKills).toBe(0);
    });

    it('片手武器でも盾を装備していれば oneHandFreeKills は加算されない', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({
      ...
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().oneHandFreeKills).toBe(0);
    });

    it('両手武器での討伐で twoHandKills が加算される', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ player: { ...player(), weapon: { item: 'spear', q: 0 }, shield: null } });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().twoHandKills).toBe(1);
      expect(masteryStore.readMastery().oneHandFreeKills).toBe(0);
    });

    it('HP満タンでの討伐で unhurtKills が加算される', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ player: { ...player(), hp: player().maxHp } });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().unhurtKills).toBe(1);
      expect(masteryStore.readMastery().lowHpKills).toBe(0);
    });

    it('HP25%以下での討伐で lowHpKills が加算される', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ player: { ...player(), hp: 1 } }); // 1/24 <= 25%
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().lowHpKills).toBe(1);
      expect(masteryStore.readMastery().unhurtKills).toBe(0);
    });

    it('「絞る」以下の明かりでの討伐で darkKills が加算される(消灯でも同様)', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ lightLevel: 0 });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().darkKills).toBe(1);
    });

    it('「普通」以上の明かりでの討伐では darkKills は加算されない', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      useRogue.setState({ lightLevel: 1 });
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().darkKills).toBe(0);
    });

    it('投げナイフでの討伐で knifeKills が加算される(fistKills には加算されない)', async () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      const b = placeBeastAdjacent('bat', 1);
      ...
      useRogue.getState().clickBeast(b.id);
      await run();
      expect(masteryStore.readMastery().knifeKills).toBe(1);
      expect(masteryStore.readMastery().fistKills).toBe(0);
    });

    it('盾の回避成功で evades が加算される(回避100%で決定的)', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      placeBeastAdjacent('bat');
    ...
    it('障壁が実際に削れた量だけ absorbed が加算される', () => {
      masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
      placeBeastAdjacent('bat');
  ...
  it('RunRecord.skills: 死亡時点の装着スキルが "id:rank" 形式で履歴に残る', () => {
    ...
    try {
      ...
      expect(useRogue.getState().phase).toBe('dead');
      const h = history.readHistory();
      expect(h[0].skills).toEqual(['kenPunch:1', 'jutsu:2']);
    } finally {
      history.setHistoryStorageForTest(null);
    }
  });

  it('セーブ v12: skillSlots/skillEquipped(ランク付き)/skillDraft/skillFreePick/cooldowns が保存・復元される(rogue-35)', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
      useRogue.setState({
        skillSlots: 4,
        skillEquipped: [{ id: 'kenPunch', rank: 1 }],
        skillDraft: [
      ...
      const s = useRogue.getState();
      expect(s.skillSlots).toBe(4);
      expect(s.skillEquipped).toEqual([{ id: 'kenPunch', rank: 1 }]);
...
describe('rogue-24: スキル配線と新カウンタ(rogue-27でランク制へ)', () => {
  it('背討ち: 未覚醒の敵へ×2ダメージ+隠密マスタリー加算(rogue-32: 攻撃命中時に加算)', async () => {
    ...
    try {
      ...
      expect(m.stealthStrikes).toBe(1); // 未覚醒への攻撃で加算(倒すかどうかは無関係)
    } finally {
      masteryStore.setMasteryStorageForTest(null);
    }
  });
  ...
  it('未覚醒の敵を素手で攻撃すると拳闘・隠密の両マスタリーに加算される(倒さなくても)', async () => {
    ...
    try {
      ...
      expect(m.stealthStrikes).toBe(1); // 攻撃時に加算
    } finally {
      masteryStore.setMasteryStorageForTest(null);
    }
  });

  it('未覚醒の敵を同じ攻撃で倒しても stealthStrikes は +1 のまま(討伐での二重加算なし)', async () => {
    masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
    try {
      ...
      expect(useRogue.getState().beasts.find((x) => x.id === b.id)!.alive).toBe(false);
      const m = masteryStore.readMastery();
      expect(m.stealthStrikes).toBe(1); // 攻撃時の1回のみ(killBeast では加算しない)
  ...
  it('覚醒済みの敵への攻撃では stealthStrikes は加算されない', async () => {
    masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
    try {
      ...
      await run(3000);
      const m = masteryStore.readMastery();
      expect(m.stealthStrikes).toBe(0);
    } finally {
      masteryStore.setMasteryStorageForTest(null);
    }
  });
  ...
  it('消灯: 心眼(shingan)装着中のみ4段階循環し、外すと「絞る」へ戻る(rogue-35: hiShobo吸収)', () => {
    useRogue.setState({ skillEquipped: [{ id: 'shingan', rank: 1 }], skillSlots: 6, lightLevel: 2 });
    ...
    useRogue.getState().unequipSkill('shingan');
    expect(useRogue.getState().lightLevel).toBe(0);
    expect(useRogue.getState().skillEquipped.some((e) => e.id === 'shingan')).toBe(false);
  });
  ...
  it('静水(seisui・keikai×kaeshi): 次の近接攻撃+2(1回で消費)', async () => {
    let sawBonus = false;
    for (let i = 1; i <= 20; i++) {
      const seed = i * 54321;
      useRogue.getState().restart(7);
      seedRogueRng(seed);
      const b1 = placeBeastAdjacent('bat', 999);
      useRogue.setState({
        player: { ...player(), weapon: { item: 'dagger', q: 0 } },
        skillEquipped: [],
        seisuiCharge: false,
      });
      useRogue.getState().clickBeast(b1.id);
      await run();
      const dmgWithout = 999 - useRogue.getState().beasts.find((x) => x.id === b1.id)!.hp;

      useRogue.getState().restart(7);
      seedRogueRng(seed);
      const b2 = placeBeastAdjacent('bat', 999);
      useRogue.setState({
        player: { ...player(), weapon: { item: 'dagger', q: 0 } },
        skillEquipped: [],
        seisuiCharge: true,
      ...
      useRogue.getState().clickBeast(b2.id);
      await run();
      const dmgWith = 999 - useRogue.getState().beasts.find((x) => x.id === b2.id)!.hp;

      expect(dmgWith).toBe(dmgWithout + 2);
      if (dmgWith === dmgWithout + 2) sawBonus = true;
      expect(useRogue.getState().seisuiCharge).toBe(false); // 1回で消費済み
    }
    expect(sawBonus).toBe(true);
  });

  it('静水(seisui)の結び発動: keikai×kaeshi 装着中に回避成功するとフラグが立つ', () => {
    placeBeastAdjacent('bat');
    useRogue.setState({
      player: { ...player(), hp: 24, shield: { item: 'shield', q: 45 } }, // 回避100%
      skillEquipped: [
        { id: 'keikai', rank: 1 },
        { id: 'kaeshi', rank: 1 },
      ],
    ...
    useRogue.getState().wait();
    expect(useRogue.getState().seisuiCharge).toBe(true);
  });
...
describe('背討ちの接近パッシブ(rogue-37)', () => {
  it('背討ち装着中: 未覚醒の敵の隣まで踏み込んでも気づかれない', () => {
    const b = placeBeastAdjacent('rat');
    b.awake = false;
    useRogue.setState({ skillEquipped: [{ id: 'shinSegiri', rank: 1 }], skillSlots: 6 });
    useRogue.getState().wait();
    const after = useRogue.getState().beasts.find((x) => x.id === b.id)!;
    expect(after.awake).toBe(false);
    expect(useRogue.getState().log.some((m) => m.includes('気づいた'))).toBe(false);
  });

  it('背討ち未装着: 同じ距離では通常どおり気づかれる', () => {
    const b = placeBeastAdjacent('rat');
    b.awake = false;
    useRogue.getState().wait();
    const after = useRogue.getState().beasts.find((x) => x.id === b.id)!;
    expect(after.awake).toBe(true);
    expect(useRogue.getState().log.some((m) => m.includes('気づいた'))).toBe(true);
  });

  it('気配感知(wisp)の敵: 背討ち装着中でも隣接すれば気づかれる', () => {
    const b = placeBeastAdjacent('wisp');
    b.awake = false;
    useRogue.setState({ skillEquipped: [{ id: 'shinSegiri', rank: 1 }], skillSlots: 6 });
    useRogue.getState().wait();
    const after = useRogue.getState().beasts.find((x) => x.id === b.id)!;
...
describe('二刀流の追撃(rogue-30)', () => {
  ...
  it('返し(kaeshi)は盾スロットの中身(kind不問)に関わらず発動する(旧・盾術IIのkindガードは撤廃)', () => {
    // rogue-30 時代は「盾スロットに左手武器が入っていると盾術IIの受け反撃が発動しない」
    // という kind ガードがあったが、rogue-35 の返し(kaeshi)は「盾でも素手でも発動」を
    // 明示的な仕様とするため、盾スロットの中身を問わない(回避成立の条件は素手/盾/背水など
    // 他スキル次第)。ここでは kenHaisui(低HPかつ障壁0で回避+25%)で回避を成立させる。
    const b = placeBeastAdjacent('bat');
    const hp0 = b.hp;
    useRogue.setState({
      player: {
        ...
        barrier: 0,
        weapon: null,
        shield: { item: 'sword', q: 0 }, // 左手武器(本物の盾ではない)
        armor: { item: 'plate', q: 20 }, // 被弾しても1ダメージに抑える安全マージン
      },
      skillEquipped: [
        { id: 'kaeshi', rank: 1 },
        { id: 'kenHaisui', rank: 1 },
    ...
    for (let i = 0; i < 300 && !evaded; i++) {
      useRogue.setState({ player: { ...player(), hp: 6, barrier: 0 } });
      seedRogueRng(i);
      useRogue.getState().wait();
      evaded = useRogue.getState().log.some((m) => m.includes('かわした'));
    }
    ...
    const beast = useRogue.getState().beasts.find((x) => x.id === b.id)!;
    expect(beast.hp).toBeLessThan(hp0); // 返しが乗っている
  });
});
...
describe('連撃(rogue-30)', () => {
  ...
  it('未覚醒の相手には両ヒットへ背討ち×2が乗る(preAwake は1回だけ捕捉して共有)', async () => {
    ...
    try {
      ...
      const m = masteryStore.readMastery();
      expect(m.stealthStrikes).toBe(1);
    } finally {
      masteryStore.setMasteryStorageForTest(null);
    }
  });
...
describe('盾打ち(tateuchi・rogue-35)', () => {
  it('隣接敵を反対方向へノックバック+固定3+盾品質ダメージ。押された先の罠が誘爆する(ずらし→罠誘爆)', async () => {
    withStraightOpenPath(1, (_dir, adjacent, farCells) => {
      const farPos = farCells[0];
      useRogue.setState({
        beasts: [
          ...useRogue.getState().beasts,
          {
            id: 900,
            kind: 'bat',
            pos: adjacent,
        ...
        traps: [{ id: 800, pos: farPos, power: 999 }], // 誘爆すれば確実に討伐できる威力
        player: { ...player(), shield: { item: 'shield', q: 2 } }, // ダメージ = 3+2 = 5(bat hp999は生存)
        skillEquipped: [{ id: 'tateuchi', rank: 1 }],
        skillSlots: 6,
      });
    });
    const turn0 = useRogue.getState().turn;
    useRogue.getState().tateuchi(900);
    await run(3000);
    expect(useRogue.getState().log.some((m) => m.includes('罠を踏んだ'))).toBe(true);
    expect(useRogue.getState().traps).toHaveLength(0); // 罠は誘爆で消費された
    const beast = useRogue.getState().beasts.find((x) => x.id === 900)!;
    expect(beast.alive).toBe(false); // 誘爆(威力999)で討伐された
    expect(useRogue.getState().turn).toBe(turn0 + 1);
    expect(cdOf(useRogue.getState().cooldowns, 'tateuchi')).toBe(5); // CD6→endTurnで1回復
  });

  it('衝波(shouha・盾打ち×硬化): ノックバックが2マスになる', async () => {
    // 敵は不動種(mushnub)にする — 直後の beastsTurn で1歩戻ってきて距離が縮む誤検知を防ぐ。
    withStraightOpenPath(2, (_dir, adjacent, _farCells) => {
      useRogue.setState({
        beasts: [
          ...useRogue.getState().beasts,
          {
        ...
        player: { ...player(), shield: { item: 'shield', q: 0 } },
        skillEquipped: [
          { id: 'tateuchi', rank: 1 },
          { id: 'kouka', rank: 1 },
        ],
    ...
    // 押し先(2マス目)のセルへ実際に移動したかを stepDist で検証する(押し先の絶対座標は問わない)。
    const before = useRogue.getState().beasts.find((x) => x.id === 900)!.pos;
    useRogue.getState().tateuchi(900);
    await run(3000);
    const after = useRogue.getState().beasts.find((x) => x.id === 900);
    expect(after).toBeDefined();
    expect(stepDist(before, after!.pos)).toBe(2); // 衝波で2マス押された
  });

  it('押し先が塞がっていれば押せず、ダメージのみ入る', async () => {
    withStraightOpenPath(1, (_dir, adjacent, farCells) => {
      const farPos = farCells[0];
      useRogue.setState({
        beasts: [
        ...
        player: { ...player(), shield: { item: 'shield', q: 2 } },
        skillEquipped: [{ id: 'tateuchi', rank: 1 }],
        skillSlots: 6,
      });
    });
    const before = useRogue.getState().beasts.find((x) => x.id === 900)!.pos;
    const hp0 = useRogue.getState().beasts.find((x) => x.id === 900)!.hp;
    useRogue.getState().tateuchi(900);
    await run(3000);
    const after = useRogue.getState().beasts.find((x) => x.id === 900)!;
    expect(after.pos).toEqual(before); // 押されていない
    expect(hp0 - after.hp).toBe(5); // ダメージ(3+盾品質2)は入る
  });

  it('盾を装備していないと発動できない', () => {
    const b = placeBeastAdjacent('bat');
    useRogue.setState({
      player: { ...player(), shield: null },
      skillEquipped: [{ id: 'tateuchi', rank: 1 }],
      skillSlots: 6,
    });
    const turn0 = useRogue.getState().turn;
    useRogue.getState().tateuchi(b.id);
    expect(useRogue.getState().turn).toBe(turn0); // 発動していない
  });
...
describe('突進(tosshin・rogue-35)', () => {
  it('dash モードに入り、直線2マス先へ移動して終点で隣接敵へ通常近接攻撃する', async () => {
    let dest: Cell = [0, 0, 0];
    withStraightOpenPath(2, (_dir, _adjacent, farCells) => {
      dest = farCells[0]; // 突進の直接の終点セル(2マス先)
      useRogue.setState({
        beasts: [
          ...useRogue.getState().beasts,
          {
        ...
        skillEquipped: [{ id: 'tosshin', rank: 1 }],
        skillSlots: 6,
      });
    });
    useRogue.getState().tosshin();
    expect(useRogue.getState().uiMode).toBe('dash');
    const turn0 = useRogue.getState().turn;
    useRogue.getState().clickCell(dest);
    await run(3000);
    expect(useRogue.getState().player.pos).toEqual(dest);
    expect(useRogue.getState().turn).toBe(turn0 + 1);
    expect(cdOf(useRogue.getState().cooldowns, 'tosshin')).toBe(5);
    const beast = useRogue.getState().beasts.find((x) => x.id === 900);
    expect(beast).toBeDefined();
    expect(beast!.hp).toBeLessThan(999); // 終点で隣接する敵へ攻撃した
  });

  it('通過不可セルがあると短縮される(1マス先までしか進めない)', () => {
    let adjacentCell: Cell = [0, 0, 0];
    withStraightOpenPath(2, (_dir, adjacent, farCells) => {
    ...
    useRogue.getState().tosshin();
    // 2マス目は敵で塞がっているので dash 候補には1マス目(adjacentCell)は出るが、
    // 2マス目(敵のいるセル)は出ない。
    const cells = dashCells(useRogue.getState());
    expect(cells.some((c) => cellKey(c) === cellKey(adjacentCell))).toBe(true);
  });

  it('敵に当てず移動だけでも良い(終点に隣接敵がいなければ攻撃なし)', async () => {
    let dest: Cell = [0, 0, 0];
    withStraightOpenPath(2, (_dir, _adjacent, farCells) => {
      dest = farCells[0];
      useRogue.setState({
    ...
    expect(useRogue.getState().player.pos).toEqual(dest);
    expect(useRogue.getState().turn).toBe(turn0 + 1);
    expect(useRogue.getState().log.some((m) => m.includes('突進した'))).toBe(true);
  });

  it('装填中(CD>0)は dash モードへ入れない', () => {
    useRogue.setState({ skillEquipped: [{ id: 'tosshin', rank: 1 }], skillSlots: 6, cooldowns: { tosshin: 3 } });
    useRogue.getState().tosshin();
    expect(useRogue.getState().uiMode).toBe('walk');
  });
});

describe('替り身(kawarimi・rogue-35)', () => {
  it('隣接敵と位置を交換する(攻撃なし)。CD8をセットしターンが1進む', async () => {
    const target = placeBeastAdjacent('bat', 999);
    const targetOldPos = target.pos;
    const playerOldPos = player().pos;
    useRogue.setState({ skillEquipped: [{ id: 'kawarimi', rank: 1 }], skillSlots: 6 });
    const turn0 = useRogue.getState().turn;
    useRogue.getState().kawarimi(target.id);
    await run(3000);
    expect(useRogue.getState().player.pos).toEqual(targetOldPos);
    const after = useRogue.getState().beasts.find((x) => x.id === target.id)!;
    expect(after.pos).toEqual(playerOldPos);
    expect(after.hp).toBe(999); // 攻撃はしていない(他の敵がいないので疑似同士討ちも起きない)
    expect(useRogue.getState().turn).toBe(turn0 + 1);
    expect(cdOf(useRogue.getState().cooldowns, 'kawarimi')).toBe(7); // CD8→endTurnで1回復
  });

  it('疑似同士討ち: 旧位置に隣接する覚醒中の近接敵(本人除く)の一撃が入れ替わった敵に落ちる', async () => {
    const target = placeBeastAdjacent('bat', 999);
    const s0 = useRogue.getState();
    const allyPos = neighbors(s0.player.pos).find(
      (c) =>
        cellKey(c) !== cellKey(target.pos) &&
        s0.dungeon.open.has(cellKey(c)) &&
        !s0.beasts.some((x) => x.alive && cellKey(x.pos) === cellKey(c)),
    );
    expect(allyPos).toBeDefined();
    useRogue.setState({
      beasts: [
        ...s0.beasts,
        {
      ...
      skillEquipped: [{ id: 'kawarimi', rank: 1 }],
      skillSlots: 6,
    });
    ...
    await run(3000);
    const after = useRogue.getState().beasts.find((x) => x.id === target.id)!;
    expect(targetHp0 - after.hp).toBe(BEASTS.bat.atk); // ally の固定攻撃力ぶん減る(乱数なし)
    // ally 自身(入れ替えた本人ではない)は攻撃者なので無傷のまま。
    expect(useRogue.getState().beasts.find((x) => x.id === 901)!.hp).toBe(999);
  });

  it('遠隔敵(ranged)は疑似同士討ちに参加しない', async () => {
    const target = placeBeastAdjacent('bat', 999);
    const s0 = useRogue.getState();
    const allyPos = neighbors(s0.player.pos).find(
      (c) =>
    ...
    useRogue.setState({
      beasts: [
        ...
        {
          id: 901,
          kind: 'mage', // ranged 持ち — 疑似同士討ち(近接の一撃)には参加しない
          pos: allyPos!,
          hp: 999,
          maxHp: 999,
          barrier: 0,
    ...
    await run(3000);
    const after = useRogue.getState().beasts.find((x) => x.id === target.id)!;
    expect(after.hp).toBe(targetHp0); // 遠隔敵は撃たない
  });

  it('未覚醒の隣接敵は疑似同士討ちに参加しない', async () => {
    const target = placeBeastAdjacent('bat', 999);
    const s0 = useRogue.getState();
    const allyPos = neighbors(s0.player.pos).find(
      (c) =>
    ...
    await run(3000);
    const after = useRogue.getState().beasts.find((x) => x.id === target.id)!;
    expect(after.hp).toBe(targetHp0); // 未覚醒の ally は参加しない
  });

  it('入れ替えた本人は疑似同士討ちの対象から除外される', async () => {
    // target 以外に隣接敵がいない状態では、target 自身が「本人」なので巻き込まれない
    // (target の hp は不変)。これは上の基本テストで確認済みだが、ここでは装填(CD)も見る。
    const target = placeBeastAdjacent('bat', 999);
    useRogue.setState({ skillEquipped: [{ id: 'kawarimi', rank: 1 }], skillSlots: 6, cooldowns: { kawarimi: 4 } });
    const turn0 = useRogue.getState().turn;
    useRogue.getState().kawarimi(target.id);
    expect(useRogue.getState().turn).toBe(turn0); // 装填中なので発動しない
  });
});
...
describe('スキルのクールダウン一般化(rogue-30)', () => {
  ...
  it('セーブ v12: cooldowns.rengeki も保存・復元される', () => {
    persist.setStorageForTest(new MemStorage() as unknown as Storage);
    try {
      useRogue.getState().restart(7);
...
describe('rogue-27 S2: 罠のスキル化(装填制)と結び効果', () => {
  ...
  it('結び「矢返し」: 遠隔攻撃の回避成功時、離れた射手にも受け反撃が届く', () => {
    masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
    const far = useRogue.getState().reach.cells.find((c) => stepDist(c, player().pos) === 2)!;
    const b: Beast = { id: 950, kind: 'mage', pos: far, hp: 999, maxHp: 999, barrier: 0, home: far, homeChamber: 0, layerFloor: -999, layerCeil: 999, awake: true, alive: true, status: null, carry: null };
    useRogue.setState({
  ...
  it('矢返しなし(返しのみ)では、遠隔攻撃を回避しても非隣接の射手へ反撃しない', () => {
    masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
    const far = useRogue.getState().reach.cells.find((c) => stepDist(c, player().pos) === 2)!;
    const b: Beast = { id: 950, kind: 'mage', pos: far, hp: 999, maxHp: 999, barrier: 0, home: far, homeChamber: 0, layerFloor: -999, layerCeil: 999, awake: true, alive: true, status: null, carry: null };
    useRogue.setState({
      beasts: [...useRogue.getState().beasts, b],
      skillEquipped: [{ id: 'kaeshi', rank: 2 }],
      skillSlots: 8,
      player: { ...player(), shield: { item: 'shield', q: 10 } },
    });
    let sawEvade = false;
...
describe('rogue-24: 遠隔攻撃・気配感知・門番', () => {
  it('術師は離れていても射線が通れば撃ってくる(遠隔攻撃)', () => {
    ...
    const b: Beast = { id: 950, kind: 'mage', pos: far!, hp: BEASTS.mage.hp, maxHp: BEASTS.mage.hp, barrier: 0, home: far!, homeChamber: 0, layerFloor: -999, layerCeil: 999, awake: true, alive: true, status: null, carry: null };
    useRogue.setState({ beasts: [...useRogue.getState().beasts, b] });
    const hp0 = player().hp;
    useRogue.getState().wait();
    expect(player().hp).toBeLessThan(hp0);
...
describe('捨てる・遺物・砕く(rogue-29)', () => {
  ...
  it('dedicateRelic(rogue-32): 支度中の装着/解除が動き、finishOutfitting で「支度を整えた。」', async () => {
    masteryStore.setMasteryStorageForTest(new MemStorage() as unknown as Storage);
    try {
      masteryStore.writeMastery({ ...INITIAL_MASTERY, fistKills: 5 }); // hasAnyMastery → 支度が開く
      useRogue.setState({
        player: { ...player(), relics: [{ item: 'royalJelly', q: 0 }] },
      });
      useRogue.getState().dedicateRelic(0);
...
describe('saveCodec v12(rogue-36)', () => {
  it('v12 でラウンドトリップ: relics が保存・復元される', () => {
