...
export type BeastKind = 'bat' | 'spider' | 'ghoul' | 'wisp' | 'drake';
...
const KINDS: BeastKind[] = ['bat', 'spider', 'ghoul', 'wisp', 'drake'];

/** 深度 D の広間に湧く種族列(0〜3体)。深いほど数が増え、強い種族に寄る。 */
export function spawnTable(depth: number, rng: () => number): BeastKind[] {
  ...
  const count = Math.min(3, 1 + Math.floor(rng() * 2) + (depth > 6 ? 1 : 0));
  const out: BeastKind[] = [];
