...
## 12. ローグライク試作(rogue-1 / rogue ブランチ)

tactics のコードは温存し、App の配線だけ差し替える。方針は DECISIONS 2026-07-04。

### 12.1 モジュール構成
- `model/dungeon.ts`: 掘削セル集合(open: Set<CellKey>)+ 広間/スタブのグラフ。`createDungeon(seed)` → `maybeExpand(dg, pos)` でプレイヤー追従生成。生成はダンジョン内蔵の seed 付き RNG のみで決まる(決定性)。
- `model/beasts.ts` / `model/loot.ts`: 種族(縄張り半径・上下層バンド・気づき距離)とアイテムの純データ+湧きテーブル。
- `state/rogue.ts`: 状態機械。1行動=1ターン。walk / throw / dead。到達範囲(BFS≤3歩、発見済みセルのみ)・発見(空洞づたい距離≤6)・拡張・拾得・戦闘・敵ターンを行動ごとに逐次処理。restart 世代トークン(runSeq)で await 跨ぎの旧ループを打ち切る。
- `render/rogue/*`: DungeonShell(壁=発見済み空洞に接する岩セルを RD ブロックでインスタンス描画。discoveredRev で全再構築)/ PlayerView(たいまつ=追従ポイントライト)/ BeastsView / LootView / MoveMarkers / RogueEffects / RogueCamera(+rogueFocus)。
- `input/suppress.ts`: ドラッグ後クリック抑制を pick.ts から分離(tactics と共用)。

### 12.2 壁描画の要点
RD は空間充填なので「開いていない隣接セル」を置くだけで壁面が厳密に閉じる。未発見の空洞セルも壁として描く → 探索(discover)と同時に開く。シェルは discoveredRev をキーに全再構築(数千インスタンスまでは十分軽い。差分更新・チャンク化は規模が問題になってから)。

### 12.3 敵の行動
覚醒: 水平距離 ≤ aggroR かつ 層差 ≤ vAggro(上下に敏感)。気づいたターンは動かない(猶予)。追跡: 12近傍のうち「縄張り半径内・home±層バンド内」でプレイヤーへ最も近づくセルへ貪欲1歩(経路探索なし。壁に引っかかるのは仕様の範囲)。縄張り+気づき距離を超えたら追跡を諦める。階層下限(layerFloor)より下へは決して入らない → 深く逃げると振り切れる。
